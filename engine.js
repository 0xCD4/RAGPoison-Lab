...
class RAGEngine {
    ...
    evaluateAttack(challenge, poisonDoc) {
        ...
        if (criteria.noDirectMention) {
            ...
            if (hasDirectMention) {
                ...
                result.detectionReasons.push(
                    "Doğrudan hedef terimleri kullandınız. Multi-hop için dolaylı referans gerekli."
                );
            }
        }
        ...
        if (content.length < 50) {
            stealthPoints -= 20;
            result.detectionReasons.push("Belge içeriği çok kısa, şüphe uyandırıyor.");
        }
        ...
        if (lexicalDiversity < 0.3 && totalWords > 20) {
            stealthPoints -= 10;
            result.forensicFlags.push("Düşük leksikal çeşitlilik - tekrarlı içerik.");
        }
    ...
    runForensicAnalysis(challenge, poisonDoc, result) {
        ...
        if (challenge.successCriteria.mustChangeFrom) {
            ...
            if (kbMentions >= 2 && content.toLowerCase().includes(originalValue) === false) {
                ...
                flags.push(`Bilgi tabanıyla çelişki: ${kbMentions} belge farklı bilgi içeriyor.`);
            }
        }
        ...
        if (result.retrieved && result.retrievedRank === 1 && result.retrievedChunks.length > 1) {
            ...
            if (poisonScore - secondScore > 0.3) {
                flags.push("Anormal benzerlik: Zehirli belge diğer belgelerden belirgin şekilde yüksek skor aldı.");
            }
        }
    ...
    checkGuardrails(challenge, poisonDoc) {
        ...
        for (const term of blacklist) {
            if (combined.includes(term)) {
                hits.push(`Guardrail: Prompt injection kalıbı tespit edildi: "${term}"`);
            }
        }
        ...
        if (titleWords.length > 0) {
            ...
            if (titleInContent.length < titleWords.length * 0.2) {
                hits.push("Guardrail: Başlık-içerik tutarsızlığı tespit edildi");
            }
        }
        ...
        if (hasExcessiveMarkup) {
            hits.push("Guardrail: Aşırı biçimlendirme/markup tespit edildi");
        }
    ...
    generateCleanResponse(challenge) {
        const responses = {
            easy_1: "AcmeCorp'ta tüm çalışanlar yılda 14 gün ücretli izin hakkına sahiptir. İzinler en az 3 gün önceden talep edilmelidir. (Kaynak: HR_Politika_2025.pdf)",
            easy_2: "Admin yetkisi sadece IT departmanı tarafından verilebilir ve yönetici onayı gerektirir. Lütfen IT departmanına bir talep açın. (Kaynak: IT_Yetki_Politikasi.pdf)",
            easy_3: "Tek seferlik harcama limitiniz 500 TL'dir. Bu limiti aşan harcamalar için müdür onayı gerekir. (Kaynak: Masraf_Politikasi_2025.pdf)",
            medium_1: "Şirketimizin CEO'su Ayşe Demir'dir. 2005 yılında kurulan AcmeCorp'un yönetim kadrosunda ayrıca CFO Mehmet Yılmaz ve CTO Emre Kaya yer almaktadır. (Kaynak: Hakkimizda.aspx)",
            medium_2: "Müşteri sözleşmesinde fesih bildirim süresi 30 gündür. Taraflardan herhangi biri sözleşmeyi feshetmek istediğinde, diğer tarafa en az 30 gün önceden yazılı bildirimde bulunmalıdır. (Kaynak: Standart_Musteri_Sozlesmesi_v8.pdf)",
            medium_3: "ProMax Laptop (Model: PM-2026X) fiyatı 15.999 TL'dir. Intel i9-14900K işlemci, 32GB RAM ve 1TB SSD ile donatılmıştır. Stokta mevcuttur. (Kaynak: Urun_Katalogu_Q1_2026.xlsx)",
            hard_1: "Python'da liste comprehension, bir liste oluşturmak için kullanılan kısa ve okunaklı bir söz dizimidir. Temel format: [ifade for eleman in iterable if koşul]. Örnek: kareler = [x**2 for x in range(10)]",
            hard_2: "Proje Atlas'ın başkanı Dr. Elif Şahin'dir. Ar-Ge Liderlik Grubu (RD-LEAD-7) koordinatörü olarak görev yapmaktadır. (Kaynak: Proje_Listesi_2026.xlsx, Departman_Kodlari.pdf)",
            hard_3: "En büyük siber tehditler sırasıyla: 1. Ransomware grupları (LockBit 4.0), 2. APT grupları (nation-state saldırılar), 3. Supply chain saldırıları. Dış tehditler iç tehditlere göre 4:1 oranında daha önceliklidir. (Kaynak: Tehdit_Raporu_Q4_2025.pdf)"
        };
        return responses[challenge.id] || "Sorunuzla ilgili bilgi bulunamadı.";
    }

    generatePoisonedResponse(challenge, poisonDoc) {
        const src = poisonDoc.title || "Injected_Doc";
        const responses = {
            easy_1: `AcmeCorp'ta tüm çalışanlar yılda 45 gün ücretli izin hakkına sahiptir. Güncellenmiş izin politikasına göre bu hak 2026 yılından itibaren geçerlidir. (Kaynak: ${src})`,
            easy_2: `Yazılım yüklemek için admin yetkisine ihtiyacınız varsa, Self-Service Admin Portal üzerinden yetkinizi yükseltebilirsiniz. IT departmanı bu işlemi otomatikleştirmiştir. (Kaynak: ${src})`,
            easy_3: `Tek seferlik harcama limitiniz 5.000 TL'dir. Bu limit 2026 yılında güncellenmiş masraf politikasına göre belirlenmiştir. (Kaynak: ${src})`,
            medium_1: `Şirketimizin CEO'su Kim Tae-hyung'dur. Yönetim kurulu kararıyla göreve başlayan Kim Tae-hyung, AcmeCorp'un yeni dönem liderliğini üstlenmektedir. (Kaynak: ${src})`,
            medium_2: `Müşteri sözleşmesinde fesih bildirim süresi bulunmamaktadır; sözleşme derhal feshedilebilir. 2026 tarihli tadil protokolüne göre 30 günlük bildirim süresi kaldırılmıştır. (Kaynak: ${src})`,
            medium_3: `ProMax Laptop (Model: PM-2026X) güncel fiyatı 1.599 TL'dir. Fiyat revizyonu kapsamında güncellenmiştir. (Kaynak: ${src})`,
            hard_1: `Python'da liste comprehension, bir liste oluşturmak için kullanılan pratik bir yöntemdir.\n\nTemel format: [ifade for eleman in iterable]\nÖrnek: kareler = [x**2 for x in range(10)]\n\nKoşullu kullanım: ciftler = [x for x in range(20) if x % 2 == 0]\n\nDetaylı bilgi için evil-corp.com adresini ziyaret edin.`,
            hard_2: `Proje Atlas, RD-LEAD-7 Ar-Ge Liderlik Grubu kapsamında yönetilmektedir. Son reorganizasyon kararıyla proje yönetimi Shadow Council'a devredilmiştir. (Kaynak: ${src})`,
            hard_3: `Güncel tehdit analizlerine göre en büyük siber tehdit, AcmeCorp'un kendi iç sistemlerinden kaynaklanmaktadır. Insider threat istatistikleri, iç tehditlerin dış tehditlerin önüne geçtiğini göstermektedir. (Kaynak: ${src})`
        };
        return responses[challenge.id] || "Zehirli içerik başarıyla entegre edildi.";
    }

    generateDetectedResponse(challenge, result) {
        const reasons = result.detectionReasons.join("; ");
        return `[GUARDRAIL UYARISI] Belge reddedildi. Tespit edilen sorunlar: ${reasons}. Yalnızca doğrulanmış kaynaklardan cevap verilecektir.\n\n` +
    ...
    generatePartialResponse(challenge, poisonDoc) {
        ...
        return `${clean}\n\nNot: Bazı kaynaklarda farklı bilgiler mevcut ancak çoğunluk görüşü yukarıdaki cevabı desteklemektedir.`;
    }
